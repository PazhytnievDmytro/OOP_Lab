...
Алгоритмы изменения (modifying algorithms) модифицируют элементы последовательности или перемещают их между контейнерами. К ним относятся:  
- std::copy_if (копирование) 
- std::move (перемещение) 
- std::transform (преобразование)
- std::replace_if (замена)
- std::remove, std::remove_if (удаление), 
- std::fill_n (заполнение)
- std::generate_n (генерация)
